...
def add_to_features(data):
    ...
    # Fill NaN values in the newly added features
...
def extract_and_reduce_features(data, n_components=3):
    ...
    imputed_trend_features = imputer.fit_transform(ta_features_trend)
    imputed_vol_features = imputer.fit_transform(ta_features_vol)
    
    pca1 = PCA(n_components=n_components)
    pca2 = PCA(n_components=min(n_components - 1, ta_features_vol.shape[1]))  # Avoid issues if n_components < number of features
...
def load_data(ticker, start_date, end_date):
    ...
    trend_features, vol_features = extract_and_reduce_features(data_with_ta, n_components=3)
